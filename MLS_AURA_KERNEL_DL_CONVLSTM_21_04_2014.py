...
loc1=('G:/research_works_vssreekanth_jrf/MY_PAPERS/paper_3a_deep_learning_for_parameter_estimation/Aura_MLS/AURA_DATA_April_2014/tr_data_pert_april_2014_mls.xlsx')
wb = xlrd.open_workbook(loc1)
sheet = wb.sheet_by_index(0)
T_pert=np.zeros((sheet.nrows,sheet.ncols));
for i in range(sheet.nrows):
...
loc1=('G:/research_works_vssreekanth_jrf/MY_PAPERS/paper_3a_deep_learning_for_parameter_estimation/Aura_MLS/AURA_DATA_April_2014/final_tst_data_21_04_2014.xlsx')
wb = xlrd.open_workbook(loc1)
sheet = wb.sheet_by_index(0)
...
for i in range(sheet.nrows):
    for j in range(sheet.ncols):
        test_data_pert[i][j]=sheet.cell_value(i,j);

###############################################################
###Normalizing the data###
#T_pert=T_pert/(np.max(T_pert));
#test_data_pert=test_data_pert/(np.max(test_data_pert));
#-----------------training the model(CNN+LSTM)-----------------

#-----reshaping train and test data----
data =T_pert.reshape(1, 128, 64, 4)#training
data_test=test_data_pert.reshape(1, 128, 64,4)# reshaping test data


#---------------------CNN model----------------------
model=Sequential()
model.add(Conv2D(32, kernel_size=(16, 16),activation='linear',input_shape=(128,64,4),padding='same'))
model.add(LeakyReLU(alpha=0.001))
model.add(MaxPooling2D((2, 2),padding='same'))
model.add(Conv2D(64, (16, 16), activation='linear',padding='same'))
model.add(LeakyReLU(alpha=0.001))
model.add(MaxPooling2D(pool_size=(2, 2),padding='same'))
model.add(Conv2D(128, (16, 16), activation='linear',padding='same'))
model.add(LeakyReLU(alpha=0.001))                  
model.add(MaxPooling2D(pool_size=(2, 2),padding='same'))
...
model_hist=model.fit(data,  y, epochs=20)
...
'''
##################_______Performance Metrics___________########################
...
###########performance metrics: T2 and SPE metrics  ####################
#----reading reconstructed signal---------

start_t= time.time();#  start time of  proposed method



loc1=('G:/research_works_vssreekanth_jrf/MY_PAPERS/paper_3a_deep_learning_for_parameter_estimation/Aura_MLS/AURA_DATA_April_2014/rbf_kernel_fmap_21_04_2014.xlsx')
wb = xlrd.open_workbook(loc1)
sheet = wb.sheet_by_index(0)
for i in range(sheet.nrows):
    for j in range(sheet.ncols):
        y_out_lstm[i][j]=sheet.cell_value(i,j);
#---SPE metric---
...
for i in range(0,127,1):
    var_train[i]=np.var(T_pert[i][1:128]);
    for j in range(0,127,1):
...
####################Limiting maximum probability to 1 ####################
for i in range(1,1,128):
    for j in range(1,1,128):
        if norm_eig_vec[i][j]>1:
            norm_eig_vec[i][j]=1;
...
ax.set_xticks(np.arange(0, 272,60))
ax.set_yticks(np.arange(30, 90, 2))
plt.xlim(0,240);
plt.ylim(70,80);
plt.clim(0, 1);'''
#---contour plot SPE-metric---

ax=plt.subplot(1,2,1)
start, stop, n_values = 13, 14, 128
start1, stop1, n_values1 = 0, 102, 128
x_vals = np.linspace(start, stop, n_values)
y_vals = np.linspace(start1, stop1, n_values1)
X, Y=np.meshgrid(x_vals, y_vals)
cp = plt.contourf(X, 9+Y,spe_metric,cmap='hsv')
plt.colorbar(cp)
#plt.clim(0, 1e-11)
ax.set_title('SPE Metric' )
ax.set_xlabel('Latitude(degrees) \n (a)')
ax.set_ylabel('Height(km)')
ax.set_xticks(np.arange(13, 14.2, .2))
ax.set_yticks(np.arange(0, 90, 2))
plt.xlim(13,14);
plt.ylim(70,80);

#---contour plot T_square-metric---
...
y_vals = np.linspace(start1, stop1, n_values1)
X, Y=np.meshgrid(x_vals, y_vals)
cp = plt.contourf(X, Y+9,T_square,cmap='hsv')
plt.colorbar(cp)
#plt.clim(0, 1e-11)
ax.set_title('$T^{2}$ Metric' )
ax.set_xlabel('Latitude(degrees) \n (b)')
ax.set_ylabel('Height(km)')
ax.set_xticks(np.arange(13, 14.2,0.2))
ax.set_yticks(np.arange(30, 90, 2))
plt.xlim(13,14);
plt.ylim(70,80);
...
###############################################################################
#######--deriving probability from probability density---########
...
##############################################################################
#--------------------implementing radial basis functions-----------------------
#----------------------kernel-1----------------------------------
import time
y=np.ones(128);
y[100:120]=np.zeros(20);
from sklearn.kernel_approximation import RBFSampler
from sklearn.linear_model import SGDClassifier

rbf_feature = RBFSampler(gamma=1, n_components=1024,random_state=1)
X_features = rbf_feature.fit_transform(y_out_lstm)
clf=SGDClassifier(max_iter=1000);
clf.fit(X_features, y);
print(clf.score(X_features, y));
stop_t=time.time();#stop time of the proposed method
print(f"Training time: {stop_t - start_t}s");
#----kernel-2 
#ax.plot([16,32,64,128,256,512,1024],[0.0159,0.0029,.0029,.0039,.0059,.0099,.0135]);
