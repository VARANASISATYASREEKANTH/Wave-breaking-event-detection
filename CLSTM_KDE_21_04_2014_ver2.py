...
for t in range(126,127,1):
    ...
    plt.figure(1)
    ax=plt.subplot(1,1,1)
    data2 =SPE_metric[t][1:59]
    density2 = kde.gaussian_kde(data2)
    x2= np.linspace(0,5000,3000)
    y2=density2(x2)
    plt.plot(x2, np.exp(y2),color='m',linewidth='3');
    plt.plot(x2, threshold[t]*np.ones(3000),color='m',linewidth='3');
    plt.title("Density Plot of the data")
    plt.show();
th_min=np.amin(threshold);
th_max=np.amax(threshold);
#---overall threshold---
