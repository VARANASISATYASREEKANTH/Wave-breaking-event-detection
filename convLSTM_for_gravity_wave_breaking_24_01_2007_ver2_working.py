...
loc1=('G:/research_works_vssreekanth_jrf/MY_PAPERS/paper_3a_deep_learning_for_parameter_estimation/programs/anamoly_detection_LSTM_RNN/test_data_24_01_2007_ver2.xlsx')
wb = xlrd.open_workbook(loc1)
sheet = wb.sheet_by_index(0)
...
eig_val,eig_vec=eig(y_out_lstm); 
eig_val_abs=np.abs(eig_val); eig_vec_abs=np.abs(eig_vec);
#-----bandpass filtering of data----
from scipy import signal
import matplotlib.pyplot as plt
sos = signal.butter(15, 30, 'hp', fs=1000, output='sos')
filtered = signal.sosfilt(sos,y_out_lstm);
#---------------------
u, s, v = np.linalg.svd(filtered, full_matrices=True)
y_normalized=preprocessing.normalize(filtered); 
y_out_lstm1=filtered/np.max(filtered);     
...
cp = plt.contourf(X, Y,eig_vec_abs,cmap='jet')
plt.colorbar(cp)
#plt.clim(0, 0.7)
ax.set_title('' )
