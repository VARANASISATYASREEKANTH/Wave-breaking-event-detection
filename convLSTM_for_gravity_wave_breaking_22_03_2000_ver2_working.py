...
u, s, v = np.linalg.svd(y_out_lstm, full_matrices=True)


y_normalized=preprocessing.normalize(y_out_lstm); 

y_out_lstm1=y_out_lstm/np.max(y_out_lstm); 
...
cp = plt.contourf(X, Y, v,cmap='jet')
plt.colorbar(cp)
#plt.clim(0, 0.015)
ax.set_title('' )
