...
loc1=('G:/research_works_vssreekanth_jrf/MY_PAPERS/paper_3a_deep_learning_for_parameter_estimation/programs/anamoly_detection_LSTM_RNN/test_data_21_04_2014_ver2.xlsx')
wb = xlrd.open_workbook(loc1)
sheet = wb.sheet_by_index(0)
...
seq.add(ConvLSTM2D(filters=1, kernel_size=(3, 3),padding='same', return_sequences=True));
seq.add(BatchNormalization());
#seq.add(Conv3D(filters=1, kernel_size=(3, 3, 3),activation='linear',padding='same', data_format='channels_last'))
...
for i in range(0,127,1):
    for j in range(0,59,1):
        SPE_metric[i][j]=np.square(y_out[i][j]-test_data_pert[i][j])/np.sqrt(np.var(y_out[i][1:60]-test_data_pert[i][1:60]));
        #SPE_metric[i][j]=y_out[i][j];


#---calculation of T2 metric---
...
plt.figure(4)
th=3.6e-3;
tot_density=np.zeros((128,60));
for q in range(100,101,1):
    data1=SPE_metric[q][1:59];
    density1=kde.gaussian_kde(data1);
    x1=np.linspace(0,5000,3000);
    y1=density1(x1);
    ax=plt.subplot(1,1,1);
    plt.plot(x1, np.exp(y1),label=str(41.9+0.3*q),linewidth='3');
    #plt.plot(x1, th*np.ones(3000),label=str(41.9+0.3*q),linewidth='3');
...
#----comparison before and after CI limit---
