...
loc1=('G:/research_works_vssreekanth_jrf/MY_PAPERS/paper_3a_deep_learning_for_parameter_estimation/programs/anamoly_detection_LSTM_RNN/mls_aura_train_data_perturbations_resampled_25km_to_80km.xlsx')
wb = xlrd.open_workbook(loc1)
sheet = wb.sheet_by_index(0)
T_pert=np.zeros((sheet.nrows,sheet.ncols));
for i in range(sheet.nrows):
...
loc1=('G:/research_works_vssreekanth_jrf/MY_PAPERS/paper_3a_deep_learning_for_parameter_estimation/programs/anamoly_detection_LSTM_RNN/AURA_test_data_21_04_2014_ver2.xlsx')
wb = xlrd.open_workbook(loc1)
sheet = wb.sheet_by_index(0)
...
data =T_pert.reshape(1, 128, 64, 250)#training
data_test=test_data_pert.reshape(1, 128, 64,250)# reshaping test data


#---------------------CNN model----------------------
model=Sequential()
model.add(Conv2D(32, kernel_size=(16, 16),activation='linear',input_shape=(128,64,250),padding='same'))
...
model_hist=model.fit(data,y, epochs=5)
