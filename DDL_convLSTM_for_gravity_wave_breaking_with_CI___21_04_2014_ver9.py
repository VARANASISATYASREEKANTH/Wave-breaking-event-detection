...
######################   After Dictionary Learning Y=DX  #########################################
loc1=('G:/research_works_vssreekanth_jrf/MY_PAPERS/paper_3a_deep_learning_for_parameter_estimation/programs/anamoly_detection_LSTM_RNN/result_21_04_2014.xlsx')
wb = xlrd.open_workbook(loc1)
sheet = wb.sheet_by_index(0)
result=np.zeros((sheet.nrows,sheet.ncols));
for i in range(sheet.nrows):
    for j in range(sheet.ncols):
        result[i][j]=sheet.cell_value(i,j);
#-----------------training the model(CNN+LSTM)-----------------

#-----reshaping train and test data----
data =T_pert.reshape(1, 128, 64, 26)#training
data_test=result.reshape(1, 128, 64, 26)# reshaping test data
...
for i in range(1,128,1):
    ...
    decoder2 = LSTM(128, activation='sigmoid', return_sequences=True)(decoder2)
