...
clear all; 
close all;
AA=xlsread('test_data_02_03_2014.xlsx');
%AA=AA1(1:184,1:61);
...
T_0_dash=mean(transpose(AA(1:184,20:30)));
...
E_p=abs(0.5.*(k./mean(N_square)).*transpose(l))*1e4;
E_p_mean=(mean(transpose(E_p)));
...
subplot(1,3,2)
plot(mean(N_square)/2,ht1,'b','linewidth',3);
hold on
...
growth_rate=exp(z./H);
subplot(1,3,3)
plot((log10(E_p_mean)),ht1,'b','linewidth',3);
ylim([50 80]);
%xticks([2:0.1:2.4])
%xlim([2 2.4]);
set(gca,'linewidth',2,'fontsize',24);
